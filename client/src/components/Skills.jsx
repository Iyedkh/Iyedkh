    ...
    <section id="skills" className="py-16 scroll-mt-24">
      ...
      <div className="flex flex-col items-center text-center mb-12">
        ...
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight text-[var(--text-primary)] leading-tight">
          {t.skills.heading1} <span className="bronze-text">{t.skills.headingHighlight}</span>
        </h2>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
        {/* Category Pills */}
        <div className="flex flex-wrap items-center gap-2 p-1.5 rounded-2xl bg-[var(--bg-surface)] border border-[var(--border)] backdrop-blur-xl w-full md:w-auto">
          {categoryTabs.map((cat) => {
            ...
            return (
              <button
                ...
                className={`relative px-4 py-2 rounded-xl text-xs font-medium transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] ${
                  isActive
                    ? "text-[var(--text-primary)] font-semibold shadow-inner"
        ...
        <div className="relative w-full md:w-64">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-secondary)] pointer-events-none" />
          <input
            type="text"
        ...
        <AnimatePresence mode="popLayout">
          {filteredSkills.map((skill) => (
            <motion.div
              ...
              className="apple-card p-5 flex flex-col justify-between group hover:border-[var(--border-strong)]"
            >
              <div>
                <div className="flex items-start justify-between mb-3.5">
                  {/* Sage technical highlight */}
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center border transition-transform duration-300 group-hover:scale-105 bg-[var(--accent-secondary-dim)] border-[var(--accent-secondary-border)] text-[var(--accent-secondary)]"
                  >
                    <SkillIcon name={skill.name} className="w-5 h-5" />
                  </div>
                  <span className="tech-pill text-[10px] py-0.5 px-2">
                    {skill.level}
                  </span>
                </div>

                <h3 className="font-bold text-base text-[var(--text-primary)] tracking-tight group-hover:text-[var(--accent-primary)] transition-colors mb-1">
                  {skill.name}
                </h3>
              ...
              <div className="mt-4 pt-3 border-t border-[var(--border)] flex items-center justify-between">
                <span className="text-[10px] font-mono text-[var(--text-secondary)] tracking-wider uppercase">
                  {skill.categoryName}
                </span>
                <span className="text-[10px] text-[var(--accent-secondary)] font-mono flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <motion.div
        ...
        className="mt-12 p-6 sm:p-8 rounded-2xl bg-[var(--bg-surface)] border border-[var(--border)]"
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 divide-y md:divide-y-0 md:divide-x divide-[var(--border)]">
          {t.skills.matrix.map((item, idx) => (
            <div key={item.title} className={`space-y-2 ${idx > 0 ? "pt-6 md:pt-0 md:pl-8" : ""}`}>
              <div className="text-[11px] font-mono font-bold tracking-widest text-[var(--accent-primary)] uppercase">
                {item.tag}
              </div>
